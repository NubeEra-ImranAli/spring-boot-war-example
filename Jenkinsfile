         ...
         stage('Generate Inventory') {
            steps {
                script {
                    ...
                    echo "\$(terraform output -raw build_server_ip) ansible_user=azureuser ansible_ssh_private_key_file=${SSH_PRIVATE_KEY_PATH} ansible_ssh_common_args='-o StrictHostKeyChecking=no'" >> inventory

                    echo "[tomcat_server]" >> inventory
                    echo "\$(terraform output -raw tomcat_server_ip) ansible_user=azureuser ansible_ssh_private_key_file=${SSH_PRIVATE_KEY_PATH} ansible_ssh_common_args='-o StrictHostKeyChecking=no'" >> inventory

                    echo "[artifact_server]" >> inventory
                    echo "\$(terraform output -raw artifact_server_ip) ansible_user=azureuser ansible_ssh_private_key_file=${SSH_PRIVATE_KEY_PATH} ansible_ssh_common_args='-o StrictHostKeyChecking=no'" >> inventory
                    """
                }
            }
        }
